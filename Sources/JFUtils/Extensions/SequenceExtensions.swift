...
// MARK: - filter
public extension Sequence {
    func filter(where keyPath: KeyPath<Element, Bool>) -> [Self.Element] {
        self.filter(where: keyPath, isEqualTo: true)
    }
    
    func filter<T: Equatable>(where keyPath: KeyPath<Element, T>, isEqualTo other: T) -> [Self.Element] {
        self.filter(by: keyPath, where: { $0 == other})
    }
    
    func filter<T: Equatable>(where keyPath: KeyPath<Element, T>, isNotEqualTo other: T) -> [Self.Element] {
        self.filter(by: keyPath, where: { $0 != other})
    }
    
    func filter<T>(by keyPath: KeyPath<Element, T>, where isIncluded: (T) -> Bool) -> [Self.Element] {
